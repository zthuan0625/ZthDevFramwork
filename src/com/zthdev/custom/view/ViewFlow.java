...
public class ViewFlow extends AdapterView<Adapter>
{
	...
	public ViewFlow(Context context, AttributeSet attrs)
	{
		...
		mSideBuffer = styledAttrs.getInt(R.styleable.ViewFlow_sidebuffer, 4);
		init();
	}
	 ...
	 * @return
	 */
	public boolean checkIsToTouch(MotionEvent ev)
	{
		switch (ev.getAction())
		{
		...
		case MotionEvent.ACTION_MOVE:
			float currentYDis = (Math.abs(ev.getY()) - oldY);
			float currentXDis = (Math.abs(ev.getX()) - oldX);
			//如果滑动的距离在X方向大于Y方向那么这是不应接受事件
			if(currentXDis > 10 && currentXDis > currentYDis)
				return true;
			break;
		}
		return false;
